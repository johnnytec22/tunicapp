                        ...
                        if (err){
                            ...
                            Teker.create(teker, function(err, added_teker){
                                if(err){
                                    ...
                                    req.flash('success', added_teker+"'s Data was Successfully Saved");
                                    res.redirect('/admin/admin_dashboard');
                                }
                            });
...
//GETTING TEKERS BY CATEGORY
route.get('/view_by_category', function(req, res) {
    ...
    if (req.session && req.session.admin){
        Admin.findOne({email : req.session.admin}, function(err, admin) {
            if (!admin){
                ...
                var new_teker_pic = req.files.display_picture;
                if (!new_teker_pic){
                    ...
                    new_teker_pic.mv('./public/images/technician_pictures/'+new_teker_pic.name, function(err) {
                        if (err){
                            req.flash('error', 'An Error occured while uploading your file, Please Try again.');
                            ...
                            var teker_updates = {
                                image_url: '/images/technician_pictures/'+new_teker_pic.name,
                                message: req.body.message,
                                firstname: req.body.firstname,
                                lastname: req.body.lastname,
                                phone: req.body.phone,
